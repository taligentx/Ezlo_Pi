...
#define ADD_PROPERTIES_DEVICE_LIST(device_id, category, sub_category, item_name, value_type, cjson_device, sensor_0010_I2C_BME680_data)                 \
    {                                                                                                                                          \
        s_ezlopi_device_properties_t *_properties = sensor_0010_I2C_BME680_sensor_prepare_properties(device_id, category, sub_category,                        \
...
static int sensor_0010_I2C_BME680_sensor_prepare(void *arg)
{
    ...
    if ((NULL != prep_arg) && (NULL != prep_arg->cjson_device) && (NULL != sensor_0010_I2C_BME680_data))
    {
        ...
        ADD_PROPERTIES_DEVICE_LIST(device_id, category_temperature, subcategory_not_defined, ezlopi_item_name_temp, value_type_temperature, prep_arg->cjson_device, sensor_0010_I2C_BME680_data);
        // device_id = ezlopi_cloud_generate_device_id();
        ADD_PROPERTIES_DEVICE_LIST(device_id, category_humidity, subcategory_not_defined, ezlopi_item_name_humidity, value_type_humidity, prep_arg->cjson_device, sensor_0010_I2C_BME680_data);
        device_id = ezlopi_cloud_generate_device_id();
        ADD_PROPERTIES_DEVICE_LIST(device_id, category_generic_sensor, subcategory_not_defined, ezlopi_item_name_atmospheric_pressure, value_type_pressure, prep_arg->cjson_device, sensor_0010_I2C_BME680_data);
        device_id = ezlopi_cloud_generate_device_id();
        ADD_PROPERTIES_DEVICE_LIST(device_id, category_generic_sensor, subcategory_not_defined, ezlopi_item_name_volatile_organic_compound_level, value_type_substance_amount, prep_arg->cjson_device, sensor_0010_I2C_BME680_data);
        device_id = ezlopi_cloud_generate_device_id();
        ADD_PROPERTIES_DEVICE_LIST(device_id, category_generic_sensor, subcategory_not_defined, ezlopi_item_name_distance, value_type_length, prep_arg->cjson_device, sensor_0010_I2C_BME680_data);
        device_id = ezlopi_cloud_generate_device_id();
        ADD_PROPERTIES_DEVICE_LIST(device_id, category_generic_sensor, subcategory_not_defined, ezlopi_item_name_co2_level, value_type_substance_amount, prep_arg->cjson_device, sensor_0010_I2C_BME680_data);        
    }
    return ret;
}
                                                                      ...
                                                                      const char *value_type, cJSON *cjson_device, bme680_data_t *sensor_0010_I2C_BME680_data)
{
    ...
    if (NULL != cjson_device)
    {
        ...
        if (bme680_properties)
        {
            ...
            bme680_properties->interface.i2c_master.channel = BME680_I2C_CHANNEL;
...
static int sensor_0010_I2C_BME680_update_values(s_ezlopi_device_properties_t *properties)
{
    ...
    {
        bme680_get_data(sensor_0010_I2C_BME680_data);
        #if 1
        TRACE_I("---------------------------------------");
        TRACE_I("Air Quality Index : %f", sensor_0010_I2C_BME680_data->iaq);
        TRACE_I("AIQ accuracy : %s", sensor_0010_I2C_BME680_data->iaq_accuracy ? "TRUE" : "FALSE");
        TRACE_I("co2_equivalent : %f", sensor_0010_I2C_BME680_data->co2_equivalent);
        TRACE_I("voc_equivalent : %f", sensor_0010_I2C_BME680_data->voc_equivalent);
        TRACE_I("Temperature : %f", sensor_0010_I2C_BME680_data->temperature);
        TRACE_I("Pressure : %f", sensor_0010_I2C_BME680_data->pressure);
        TRACE_I("Humidity : %f", sensor_0010_I2C_BME680_data->humidity);
        TRACE_I("Gas resistance : %f", sensor_0010_I2C_BME680_data->gas_resistance);
        TRACE_I("Altitude : %f", sensor_0010_I2C_BME680_data->altitude);
        TRACE_I("Stabilization_status : %s", sensor_0010_I2C_BME680_data->stabilization_status ? "TRUE" : "FALSE");
        TRACE_I("run in status : %s", sensor_0010_I2C_BME680_data->run_in_status ? "TRUE" : "FALSE");
        TRACE_I("---------------------------------------");
        #endif
    }
    return ret;
...
static int sensor_0010_I2C_BME680_get_value_cjson(s_ezlopi_device_properties_t *properties, void *args)
{
    ...
    if (cjson_properties && sensor_0010_I2C_BME680_data)
    {
        ...
        if (ezlopi_item_name_atmospheric_pressure == properties->ezlopi_cloud.item_name)
        {
            ...
            cJSON_AddStringToObject(cjson_properties, "valueFormatted", formatted_value);
            cJSON_AddNumberToObject(cjson_properties, "value", (uint16_t)(sensor_0010_I2C_BME680_data->pressure));
